...
struct Block {
  ...
  : local_depth{0}
  { 
    cout << "split testing" << endl;
    __= pmemobj_tx_zalloc(sizeof(Pair) * kNumSlot, TOID_TYPE_NUM(char));
  ...
  :local_depth{depth}
  { 
    //cout << "new Block with depth " << depth << endl;
    __ = pmemobj_tx_zalloc(sizeof(Pair) * kNumSlot, TOID_TYPE_NUM(char));
    _ = (Pair *)cache_align(pmemobj_direct(__));
    for (int j = 0; j <  kNumSlot ; j++){
          _[j].key = INVALID;
...
class ExtendibleHash : public hash_api {
  ...
  void print_all(){
    ...
    for(int i = 0; i < dir->capacity; i ++){
      cout << "The record in block " << i << " is " << endl;
      for(int j = 0; j < dir->_[i]->kNumSlot;j++){
        cout << "The record in slot " << j << " is " << endl;
        if(dir->_[i]->_[j].key != INVALID){
          cout << dir->_[i]->_[j].key << endl;
          counter ++;
        }
      }
    }
    cout << "The number of record is " << counter << endl;
  }
  PMEMobjpool *pop;
  size_t global_depth;
  Directory *dir;
