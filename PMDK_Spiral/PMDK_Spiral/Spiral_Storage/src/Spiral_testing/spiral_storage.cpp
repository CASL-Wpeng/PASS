...
void displayBits(uint64_t value) {
    for (int i = 63; i >= 0; --i) {
        std::cout << ((value >> i) & 1);
        if (i % 8 == 0) {
            std::cout << ' ';  // Add a space between groups of 8 bits
        }
    }
    std::cout << std::endl;
}

// Function to display bits for a char array
void displayBitsForCharArray(const char* charArray, std::size_t size) {
    for (std::size_t i = 0; i < size; ++i) {
        displayBits(charArray[i]);
        std::cout << ' ';  // Add a space between char bits for better readability
    }
    std::cout << std::endl;
}
...
int main(int argc, char *argv[]){
  ...
  long double myLongDouble = 0.156474152456498;
  ...
  std::cout <<"The bits converted back from told() is" << std::endl;
  ...
  memcpy(myStringKey->key, converter2.bits, 16);

  memcpy(converter2.bits, myStringKey->key, 16);

  showBits(converter2.ld);

  long double my_pi = reinterpret_cast<long double&>(converter2.bits);

  showBits(my_pi);

  uint64_t test = reinterpret_cast<uint64_t&>(myStringKey->key);
