...
RETRY:
  ...
  uint32_t lock_idx = loc / locksize;
  ...
  while (pmemobj_rwlock_trywrlock(pop, &mutex[lock_idx]) != 0) {
      ...
      if (resizing == true) {
        ...
        goto RETRY;
          ...
          pmemobj_persist(pop, &dict[loc]->slot[i], sizeof(Entry));
          item_number ++;
...
bool LinearHash::Positive_Get(Key_t& key) {
RETRY:
  while (resizing_lock.load() == 1) {
    asm("nop");
  }
  ...
  for (int i = 0; i < ASSOC_NUM; i++)
  {
    if (dict[key_hash]->slot[i].key == key || dict[key_hash]->slot[i].key != key){
      //cout << "find key " << key << " location is " << key_hash << " hash factor is " << hash_factor << endl;
      Value_t get_value = dict[key_hash]->slot[i].value;
      pmemobj_rwlock_unlock(pop, &mutex[key_hash / locksize]);
      return true;
...
bool LinearHash::Negative_Get(Key_t& key) {
RETRY:
  while (resizing_lock.load() == 1) {
    asm("nop");
  }
...
bool LinearHash::Recovery(){
  TX_BEGIN(pop){
    pmemobj_tx_add_range_direct(this,sizeof(class LinearHash));
    int item_num = 0;
  for(int i = 0; i < size;i++){
    for(int j = 0; j < ASSOC_NUM;j++){
      if(dict[i]->slot[j].key!=INVALID){
        item_num++;
      }
    }
  }
