...
uint8_t logical_to_physical_testing(uint8_t logical){
    uint8_t high = (1 + logical)/2;
    uint8_t low = logical/2;
    uint8_t physical;
    if (low < high){
        physical = logical_to_physical_testing(low);
    }
    else {
...
// 	for(int i = 16000000; i < 16001000; i ++){
// 		int result = logical_to_physical_testing(i);
// 	}
//clock_gettime(CLOCK_MONOTONIC, &finish);
  ...
  int look_up_table_testing[1000];
  for(int i = 0; i < 1000; i++){
    look_up_table_testing[i] = logical_to_physical_testing(i);
  }
  int d = 0;
  clock_gettime(CLOCK_MONOTONIC, &start);
    for(int i = 0; i < 1000; i ++){
		int result = d + pow(2,rand()/RAND_MAX);
    int physical = look_up_table_testing[result];
	}
  clock_gettime(CLOCK_MONOTONIC, &finish);
  single_time = (finish.tv_sec - start.tv_sec) + (finish.tv_nsec - start.tv_nsec) / 1000000000.0;
  cout << "time usage is " <<single_time<< endl;
