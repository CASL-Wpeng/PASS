...
// hash key
	...
	uint64_t hash_key[100000000] = {0};

	for(int i = 0; i < 10000;i++){
		for(int j = 0; j < 10000; j++){
			hash_key[i*10000 +j] = rand();
		}
	}
...
//spiral storage with power table
   ...
   int init = 1;
   uint32_t power_table[65536*16] = {0};
...
//spiral storage with all
    ...
    for(int i = 0;i < 65536*16;i++){
        ...
        power_table2[i] = logical_to_physical(init - 1);
    }
